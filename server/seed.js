  ...
  await db.transaction(async (tx) => {
    for (const row of rows) {
      ...
      const { lastInsertRowid: movieId } = await tx.run(
        `INSERT INTO movies (director, title, year, rank_global, mn, watchlist, tokens, token_pts, imdb_id, imdb_rating, poster_path, runtime)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        row.director || '',
        row.movie || '',
        row.year || '',
        ...
        // Present in seeds regenerated by scripts/backup-and-seed.js; absent in
        ...
        row.runtime ? parseInt(row.runtime, 10) : null,
      );
      ...
      for (const voter of row.watchlistVotes || []) {
        await tx.run('INSERT OR IGNORE INTO watchlist_votes (movie_id, voter) VALUES (?, ?)', movieId, voter);
      }
    }
  });
