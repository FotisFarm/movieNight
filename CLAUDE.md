...
- **Model**: `gemini-2.5-flash` (configurable via `GEMINI_MODEL`) using the `@google/genai` SDK (`server/llm.js`), persona = HAL 9000. `thinkingBudget: 0` is set to keep responses fast and snappy.
- **Cards**: when an answer lists films/directors/years/decades, HAL appends a fenced ` ```cards ` block holding a JSON array (`{ type, id?, value?, title, meta, score, scoreLabel }`). `HalDrawer.jsx` and `Chat.jsx` parse that block out of the reply (`parseHalReply`) and render cards styled with score badges and highlights. Every card is clickable: `movie` cards open `MovieModal`; `director`/`year`/`decade` cards open `DirectorYearModal`. Requires `GEMINI_API_KEY` (or fallback `GOOGLE_API_KEY`) in `.env`. If unset, returns a graceful "chat unavailable" message.
...
- **Flow**: `POST /api/chat { messages: [{role, content}] }` → `server/routes/chat.js` (behind `requireAuth`, passes `req.session.voter` so "what should I watch" is personalised) → `llm.chat()` with multi-turn tool calling loop → `{ reply }`. Errors are returned as a reply bubble, not a 5xx.
