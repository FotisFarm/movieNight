...
🔒 **Hidden UI Entry Points (Easter Egg & Private Console)**:
HAL is kept hidden from the general group so other members don't see it prematurely:
1. **Global Shortcut**: Press `Ctrl+K` or `Cmd+K` (or `Ctrl+Shift+H`) anywhere in the app to summon the slide-over HAL 9000 console drawer (`HalDrawer.jsx`).
2. **Easter Egg Click**: Rapidly triple-clicking the "🎬 Movie Night" header logo reveals and toggles HAL.
3. **URL Parameter**: Visiting any page with `?hal=1` unlocks and opens the HAL console.
4. **Local Browser Unlock State**: When unlocked, `localStorage.getItem('mn-hal-unlocked')` persists the glowing red HAL-eye SVG button in `header-right` exclusively on your machine. Clicking the lock icon in the HAL drawer header immediately re-hides it from the header.

- **Model**: `gemini-3.6-flash` (configurable via `GEMINI_MODEL`) using the `@google/genai` SDK (`server/llm.js`), persona = HAL 9000.
- **Cards**: when an answer lists films/directors/years/decades, HAL appends a fenced ` ```cards ` block holding a JSON array (`{ type, id?, value?, title, meta, score, scoreLabel, poster_path?, runtime?, letterboxd_rating? }`). `HalDrawer.jsx` and `Chat.jsx` parse that block out of the reply (`parseHalReply`) and render cards with TMDb poster thumbnails, runtime tags, Letterboxd rating badges, and score highlights. Every card is clickable: `movie` cards open `MovieModal`; `director`/`year`/`decade` cards open `DirectorYearModal`.
- **SQL Telemetry**: executed SQL queries during tool turns are tracked and returned with `{ reply, queries }`, rendered in an expandable "⚡ Inspect SQL" telemetry accordion under assistant messages.
- **Waveform & Wide Console**: header includes an animated oscilloscope waveform (`HalWaveform.jsx`) and a width toggle between compact (490px) and wide console mode (780px).
- **Suggestions & Categorized Prompts**: welcome screen provides categorized inquiry tabs (Tonight, Rankings, Debate, Reviews), and HAL optionally suggests contextual follow-up question chips (` ```suggestions ` block).
...
- **`movie_scores`** (a permanent `VIEW` in the main schema, shared by both connections — see [Database / persistence](#database--persistence-turso)) exposes the derived scores that otherwise live only in JS (`enrichMovie`): `voter_count`, `fair_score`, `boost`, `fair_boosted`, `boosted_score`, `std_dev`. Base tables documented in system prompt also include `runtime`, `letterboxd_rating`, `poster_path`, `lists`, and `ratings.comment`.
...
- **Flow**: `POST /api/chat { messages: [{role, content}] }` → `server/routes/chat.js` (behind `requireAuth`, passes `req.session.voter` so "what should I watch" is personalised) → `llm.chat()` with multi-turn tool calling loop → `{ reply, queries }`. Errors are returned as a reply bubble, not a 5xx.
- **Env**: `GEMINI_API_KEY` is forwarded in `docker-compose.yml` and loaded from the root `.env` (gitignored).
