...
- **Cards**: when an answer lists films/directors/years, HAL appends a fenced ` ```cards ` block holding a JSON array (`{ type, id?, title, meta, score, scoreLabel }`). `Chat.jsx` parses that block out of the reply (`parseReply`) and renders a **column of cards** styled like the Stats-page highlight tiles (`hal-card*` classes). Movie cards carry `movies.id` and are clickable → open the shared `MovieModal`. Requires `ANTHROPIC_API_KEY` in the root `.env` (loaded by dotenv). If the key is unset, `/api/chat` returns a graceful "chat unavailable" reply instead of erroring — the rest of the app is unaffected.
